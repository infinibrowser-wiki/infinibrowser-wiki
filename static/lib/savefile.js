...
const pair2int = (a, b) => a > b
  ? BigInt(a) << 24n | BigInt(b)
  : BigInt(b) << 24n | BigInt(a);
...
class SaveFile {
  ...
  static async decode(raw, options) {
    ...
    if (type == "official") return await new SaveFile(options).decodeOfficial(raw);
    else if (type == "binaryV2") return await new SaveFile(options).decodeBinaryV2(raw);
    else if (type == "binaryV1") return await new SaveFile(options).decodeBinaryV1(raw);
    else if (type == "legacy") return await new SaveFile(options).decodeLegacy(raw);
  ...
  async decodeBinaryV1(raw) {
    ...
    for (let id = 0; id < elementCount; id++) {
      ...
      for (let i = 0; i < recipeCount; i++) {
        let a = decodeLEB128(read), b = decodeLEB128(read) + a;
        list.push([a, b]);
      }
    }
    ...
    for (const element of this.elements) element.emoji = emojis.get(element.emoji) || DEFAULT_EMOJI;
    ...
    for (const [element, list] of recipes) {
      ...
      for (const recipe of list) {
        const a = this.elements[recipe[0]], b = this.elements[recipe[1]];
        if (!a || !b) continue;
  ...
  decodeLegacy(raw) {
    ...
    for (const text in data.recipes) {
      ...
      if (!Array.isArray(recipes) || recipes.length < 1 || text == "Nothing") continue;
